...
export default function ConfirmOrder() {
  ...
  const advance = async (status, message) => {
    setBusy(true);
    try {
      await orderService.updateStatus(orderId, status, { actor: 'Producer' });
  ...
  const markReady = async () => {
    await advance('READY_FOR_SHIPMENT', 'Order is ready for pickup or shipment.');
  };

  if (loading) return <Loading label="Loading order…" />;
  if (!order) return <PageHeaderBlank />;

  return (
    <div className="space-y-6">
      <Button asChild variant="ghost" size="sm" className="gap-1 text-muted-foreground">
        <Link to="/producer/farmer/orders/incoming"><ArrowLeft className="h-4 w-4" /> Back to orders</Link>
      </Button>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {['PROCESSING'].includes(order.status) ? (
              <Button variant="outline" className="w-full gap-2" disabled={busy} onClick={markReady}>
...
function PageHeaderBlank() {
  return (
    <Card><CardContent className="py-16 text-center text-muted-foreground">Order not found.</CardContent></Card>
  );
}