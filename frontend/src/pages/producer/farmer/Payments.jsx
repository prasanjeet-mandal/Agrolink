...
export default function Payments() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (p) => (
        <Link to="/orders" className="text-xs text-muted-foreground hover:text-foreground">{p.orderNumber}</Link>
      ),
    },
    {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Payments"
        description="Track settlements from your orders."
        actions={
          <Button asChild variant="outline" className="gap-2">
            <Link to="/payment/history"><Wallet className="h-4 w-4" /> Full history</Link>
          </Button>
        }
      />

      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="Cleared earnings" value={formatPrice(stats.totalRevenue ?? 0, { compact: true })} icon={Banknote} hint="released to your bank" />
        <StatCard label="Awaiting settlement" value={formatPrice(pendingTotal, { compact: true })} icon={Clock} hint="held in escrow / processing" />
        <StatCard label="Transactions" value={payments.length} icon={Wallet} />
      </div>
      ...
      <DataTable
        ...
        empty={{
          ...
          description: 'Completed orders automatically generate a settlement.',
        }}
      />
    </div>
  );
