...
@Service public class ProductServiceImpl implements ProductService {
 ...
 public List<ProductResponse> mine(String email){return products.findBySellerEmail(email).stream().map(this::to).toList();}
 public ProductResponse get(Long id){return to(products.findById(id).orElseThrow(()->new RuntimeException("Product not found")));}
 ...
 private void set(Product p,ProductRequest r){p.setName(r.name());p.setDescription(r.description());p.setPrice(r.price());p.setUnit(r.unit());p.setAvailableQuantity(r.availableQuantity());p.setImageUrl(r.imageUrl());p.setLocation(r.location());p.setStatus(ProductStatus.ACTIVE);}
 ...
 private ProductResponse to(Product p){return new ProductResponse(p.getId(),p.getSeller().getId(),p.getSeller().getFullName(),p.getCategory().getId(),p.getCategory().getName(),p.getName(),p.getDescription(),p.getPrice(),p.getUnit(),p.getAvailableQuantity(),p.getImageUrl(),p.getLocation(),p.getStatus().name(),p.getSeller().getRole().name());}
}
