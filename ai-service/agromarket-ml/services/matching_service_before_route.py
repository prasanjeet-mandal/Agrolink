...
def calculate_matching_score(distance_km):
...
def find_matching_supply(
    ...
    for location in supply_result["locations"]:
        ...
        item["distance_available"] = False
        item["distance_km"] = None
        item["matching_score"] = None
        item["reason"] = "Distance unavailable"
        ...
        if destination:
            ...
            if distance_result["available"]:

                distance_km = distance_result["distance_km"]

                item["distance_available"] = True
                item["distance_km"] = distance_km

                item["matching_score"] = calculate_matching_score(
                    distance_km
                )

                if distance_km == 0:
                    item["reason"] = "Same location as destination"
                else:
                    item["reason"] = "Nearest available supply location"
    ...
    if destination:
        ...
        locations.sort(
            key=lambda x: (
                x["matching_score"] is None,
                -(x["matching_score"] or 0)
            )
        )
